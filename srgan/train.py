...
def main():
  ...
  parser.add_argument('--log-freq', type=int, default=100, help='How many training iterations between validation/checkpoints.')
  ...
  with tf.Session() as sess:
    ...
    while True:
      if iteration % args.log_freq == 0:
        ...
        # Log error
        print('[%d] Test: %.7f, Train: %.7f' % (iteration, val_error, eval_error), end='', flush=True)
        # Evaluate benchmarks
        log_line = ''
        for benchmark in benchmarks:
          psnr, ssim, _, _ = benchmark.evaluate(sess, g_y_pred, log_path, iteration)
          print(' [%s] PSNR: %.2f, SSIM: %.4f' %( benchmark.name, psnr, ssim), end='', flush=True)
